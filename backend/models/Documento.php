...
class Documento {
    ...
    public function findByCategoria($categoriaId, $clienteId = null) {
        if ($clienteId !== null) {
            return $this->db->select('documentos', '*', 'categoria_id = ? AND cliente_id = ?',
                [$categoriaId, $clienteId], 'fecha_subida DESC');
        } else {
            return $this->db->select('documentos', '*', 'categoria_id = ?',
                [$categoriaId], 'fecha_subida DESC');
        }
    }

    public function findById($id) {
        return $this->db->selectOne('documentos', '*', 'id = ?', [$id]);
    }
    ...
    public function getTodos($clienteId = null) {
        if ($clienteId !== null) {
            return $this->db->select('documentos', '*', 'cliente_id = ?',
                [$clienteId], 'fecha_subida DESC');
        } else {
    ...
    public function getPorRuta($ruta) {
        return $this->db->selectOne('documentos', '*', 'ruta_archivo = ?', [$ruta]);
    }
