...
class AdminController {
    ...
    public function stats() {
        ...
        try {
            ...
            $stats = [
                ...
                'clientes' => $this->getTotalClientes(),
                'documentos' => $this->getTotalTransacciones(),
                'categorias' => $this->getTotalCategorias()
            ];
    ...
    private function getTotalUsuarios() {
        try {
            $usuarios = $this->db->select('usuarios', '*', 'is_active = ?', [true]);
            error_log("getTotalUsuarios: encontrados " . count($usuarios) . " usuarios");
            return count($usuarios);
        } catch (Exception $e) {
            error_log("Error getTotalUsuarios: " . $e->getMessage());
            return 0;
        }
    }
    ...
    private function getTotalClientes() {
        try {
            $clientes = $this->db->select('clientes', '*');
            return count($clientes);
        } catch (Exception $e) {
            error_log("Error getTotalClientes: " . $e->getMessage());
            return 0;
        }
    }
    ...
    private function getTotalCategorias() {
        try {
            $categorias = $this->db->select('tipos_documento', '*', 'is_active = ?', [true]);
    ...
    private function getTotalTransacciones() {
        try {
            $transacciones = $this->db->select('transacciones', '*');
            return count($transacciones);
        } catch (Exception $e) {
            error_log("Error getTotalTransacciones: " . $e->getMessage());
            return 0;
        }
    }
}
