...
class ClientesController {
    ...
    public function crear() {
        ...
        try {
            // Si se proporcionan datos de usuario, crear usuario asociado
            $userId = null;
            if (!empty($input['crear_usuario']) && !empty($input['username']) && !empty($input['password'])) {
            ...
            $clienteData = [
                ...
                'rut_empresa' => trim($input['rut_empresa']),
            ...
            // Mapear campos adicionales según la estructura de la BD
            if (isset($input['nombre_cliente'])) {
                $clienteData['nombre_cliente'] = trim($input['nombre_cliente']);
            }
            if (isset($input['empresa'])) {
                $clienteData['empresa'] = trim($input['empresa']);
            }
            if (isset($input['correo_contacto'])) {
                $clienteData['correo_contacto'] = trim(strtolower($input['correo_contacto']));
            }

            $clienteId = $this->cliente->create($clienteData);
            ...
            if ($clienteId) {
                ...
                echo json_encode([
                    ...
                    'data' => $clienteCreado,
                    'user_id' => $userId,
                    'message' => 'Cliente creado exitosamente' . ($userId ? ' con usuario asociado' : '')
                ]);
            } else {
                http_response_code(400);
                echo json_encode([
   ...
   public function eliminar($id) {
        error_log("=== ClientesController::eliminar INICIADO ===");
        error_log("Cliente ID a eliminar: " . $id);
        error_log("REQUEST_METHOD: " . $_SERVER['REQUEST_METHOD']);

        if (!$this->verifyAuth()) {
            error_log("Falló verifyAuth()");
            return;
        }
        ...
        try {
            ...
            $adminUser = $userModel->findByUsername($userData['username']);

            if (!$adminUser || !$userModel->verifyPassword($adminUser, $input['admin_password'])) {
                http_response_code(403);
                echo json_encode([
                    'success' => false,
            ...
            error_log("ClientesController::eliminar - Intentando eliminar cliente: " . json_encode($cliente));

            // Eliminar cliente
            $eliminado = $this->cliente->delete($id);

            if ($eliminado) {
                error_log("ClientesController::eliminar - Cliente eliminado exitosamente");
                echo json_encode([
                    'success' => true,
                    'message' => 'Cliente eliminado exitosamente'
                ]);
            } else {
                error_log("ClientesController::eliminar - Error: delete() retornó false");
                http_response_code(400);
                echo json_encode([
                    'success' => false,
            ...
            error_log("ClientesController::eliminar - ERROR: " . $e->getMessage());
            error_log("ClientesController::eliminar - TRACE: " . $e->getTraceAsString());
            http_response_code(500);
            echo json_encode([
                'success' => false,
