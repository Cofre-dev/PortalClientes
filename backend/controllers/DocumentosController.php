...
class DocumentosController {
    ...
    public function upload() {
        try {
            ...
            if ($documentoId) {
                ...
                $this->transaccionModel->registrar(
                    'upload',
                    $documentoId,
                    $userData['id'] ?? null,
                    $clienteId,
    ...
    public function download() {
        try {
            ...
            // Registrar transacción de descarga
            if ($documento) {
                $this->transaccionModel->registrar(
                    'download',
                    $documento['id'] ?? null,
    ...
    public function eliminar($id) {
        try {
            ...
            // Registrar transacción antes de eliminar
            $this->transaccionModel->registrar(
                'delete',
                $id,
                $userData['id'] ?? null,
                $documento['cliente_id'] ?? null,
                'Archivo eliminado: ' . $documento['nombre_archivo']
            );
        ...
        try {
            ...
            if ($updated) {
                ...
                $this->transaccionModel->registrar(
                    ...
                    $userData['id'] ?? null,
                    $documento['cliente_id'] ?? null,
                    'De: "' . $documento['nombre_archivo'] . '" a: "' . $nuevoNombreCompleto . '"'
                );
