...
<body class="<?= $bodyClass ?>">
    <div class="client-dashboard">
            ...
            <div class="dashboard-container">
                <div class="dashboard-header">
                    ...
                    <div class="header-actions">
                        <button id="helpBtn" class="btn btn-warning" style="margin-right: 0.5rem;">
        ...
        <div id="helpModal" class="modal">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <!-- Opciones de ayuda -->
                    <div class="help-options" style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1.5rem;">
                        <button id="viewConsultasBtn" class="help-option-btn" style="padding: 1.5rem; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none; border-radius: 8px; cursor: pointer; transition: all 0.3s ease; text-align: left;">
                            <i class="fas fa-comments" style="font-size: 2rem; margin-bottom: 0.5rem; display: block;"></i>
                            <strong style="font-size: 1.1rem; display: block; margin-bottom: 0.3rem;">Mis Consultas</strong>
                            <span style="font-size: 0.9rem; opacity: 0.9;">Ver estado de mis consultas</span>
                        </button>
                        <button id="newConsultaBtn" class="help-option-btn" style="padding: 1.5rem; background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); color: white; border: none; border-radius: 8px; cursor: pointer; transition: all 0.3s ease; text-align: left;">
                            <i class="fas fa-paper-plane" style="font-size: 2rem; margin-bottom: 0.5rem; display: block;"></i>
                            <strong style="font-size: 1.1rem; display: block; margin-bottom: 0.3rem;">Nueva Consulta</strong>
                            <span style="font-size: 0.9rem; opacity: 0.9;">Enviar una nueva consulta</span>
                        </button>
                    </div>

                    <!-- Formulario de nueva consulta (oculto inicialmente) -->
                    <div id="helpFormContainer" style="display: none;">
                        <div style="margin-bottom: 1rem;">
                            <button id="backToHelpOptions" class="btn btn-outline btn-sm">
                                <i class="fas fa-arrow-left"></i> Volver
                            </button>
                        </div>
                        <form id="helpForm">
                            <div class="form-group">
                                <label for="helpAsunto">Asunto *</label>
                                <input type="text" id="helpAsunto" placeholder="Breve descripción del problema o consulta" required maxlength="200">
                            </div>
                            <div class="form-group">
                                <label for="helpMensaje">Mensaje *</label>
                                <textarea id="helpMensaje" placeholder="Describe tu consulta o problema con detalle..." rows="5" required maxlength="1000"></textarea>
                            </div>
                            <div class="modal-footer" style="margin-top: 1rem; padding-top: 1rem; border-top: 1px solid #eee;">
                                <button type="button" class="btn btn-outline" data-action="cancel">Cancelar</button>
        ...
        <div id="consultasModal" class="modal">
            <div class="modal-content large">
                <div class="modal-header">
                    <h3><i class="fas fa-comments"></i> Mis Consultas</h3>
                    <div class="modal-actions">
                        <button id="refreshConsultasBtn" class="btn btn-outline btn-sm">
                            <i class="fas fa-sync-alt"></i> Actualizar
                        </button>
                        <button class="modal-close">&times;</button>
                    </div>
            ...
            <div class="modal-content" style="max-width: 900px; height: 80vh; display: flex; flex-direction: column;">
                <div class="modal-header" style="flex-shrink: 0;">
                    <div>
                        ...
                        <p style="margin: 0.5rem 0 0 0; font-size: 0.9rem; color: #666;">Chat con el equipo de soporte</p>
                    </div>
                    <button class="modal-close" onclick="clientDashboard.cerrarChatConsulta()">&times;</button>
                </div>
                ...
                <div class="modal-footer" style="flex-shrink: 0; display: flex; gap: 0.5rem; padding: 1rem; border-top: 2px solid #eee;">
                    <textarea id="chatNuevoMensaje" placeholder="Escribe tu mensaje..." style="flex: 1; padding: 0.75rem; border: 1px solid #ddd; border-radius: 6px; resize: none; font-family: inherit;" rows="2"></textarea>
                    <button onclick="clientDashboard.enviarMensajeChat()" class="btn btn-primary" style="align-self: flex-end;">
    ...
    class ClientDashboard {
        ...
        bindEvents() {
            ...
            // View Consultas button (dentro del modal de ayuda)
            document.getElementById('viewConsultasBtn').addEventListener('click', () => {
                this.hideHelpModal();
                this.showConsultasModal();
            });

            // New Consulta button (dentro del modal de ayuda)
            document.getElementById('newConsultaBtn').addEventListener('click', () => {
                this.showNewConsultaForm();
            });

            // Back to help options button
            document.getElementById('backToHelpOptions').addEventListener('click', () => {
                this.backToHelpOptions();
            });
            ...
            // Consultas modal
            document.getElementById('consultasModal').addEventListener('click', (e) => {
                if (e.target.classList.contains('modal') || e.target.classList.contains('modal-close')) {
                    this.hideConsultasModal();
                }
            });

            // Refresh consultas button
            document.getElementById('refreshConsultasBtn').addEventListener('click', () => {
                this.loadConsultas();
            });
        }
        ...
        async loadCategories() {
            ...
            try {
                if (!this.userInfo.cliente_id) {
                    throw new Error('No se encontró información del cliente');
                }

                const response = await makeRequest(`${this.baseURL}/clientes/${this.userInfo.cliente_id}/categorias`);
        ...
        async createCategory() {
            ...
            if (!this.userInfo.cliente_id) {
                showNotification('No se encontró información del cliente', 'error');
                return;
            }
            ...
            try {
                await makeRequest(`${this.baseURL}/clientes/${this.userInfo.cliente_id}/categorias`, {
                    method: 'POST',
                    body: JSON.stringify({
                        nombre,
        ...
        async submitHelp() {
            const asunto = document.getElementById('helpAsunto').value.trim();
            ...
            try {
                await makeRequest(`${this.baseURL}/consultas/crear`, {
                    method: 'POST',
                    body: JSON.stringify({
                        asunto,
                        mensaje
                ...
                showNotification('Consulta enviada exitosamente. Pronto recibirás respuesta.', 'success');
                this.hideHelpModal();
            } catch (error) {
                showNotification(error.message, 'error');
            }
        ...
        showHelpModal() {
            showModal('helpModal');
            // Mostrar opciones y ocultar formulario
        ...
        showNewConsultaForm() {
            ...
            document.getElementById('helpFormContainer').style.display = 'block';
        }
        ...
        async loadConsultas() {
            try {
                ...
                if (!response || !response.data || response.data.length === 0) {
                    container.innerHTML = `
                        <div class="empty-consultas">
                            ...
                            <p>Aún no has enviado ninguna consulta. Haz clic en "Ayuda" para enviar tu primera consulta.</p>
                        </div>
                    `;
                    return;
                }
                ...
                container.innerHTML = consultas.map(consulta => this.createConsultaCard(consulta)).join('');
                ...
                document.getElementById('consultasContainer').innerHTML = `
                    <div class="empty-consultas" style="color: #dc3545;">
        ...
        createConsultaCard(consulta) {
            const estadoBadge = consulta.estado === 'pendiente'
                ? '<span class="badge badge-warning"><i class="fas fa-clock"></i> Pendiente</span>'
                : consulta.estado === 'resuelto'
                ? '<span class="badge badge-success"><i class="fas fa-check-circle"></i> Resuelto</span>'
                : '<span class="badge badge-info"><i class="fas fa-hourglass-half"></i> En Proceso</span>';

            return `
                <div class="consulta-card" style="cursor: pointer;" onclick="clientDashboard.abrirChatConsulta(${consulta.id}, '${escapeHtml(consulta.asunto).replace(/'/g, "\\'")}')">
                    <div class="consulta-header">
                        <div class="consulta-title">
                            <h4><i class="fas fa-comments"></i> ${escapeHtml(consulta.asunto)}</h4>
                    ...
                    <div class="consulta-body">
                        ...
                        <p>${escapeHtml(consulta.mensaje)}</p>
                    </div>
                    ${consulta.estado === 'resuelto' ? `
                        <div style="margin-top: 1rem; padding: 0.75rem; background: #d4edda; border-left: 4px solid #28a745; border-radius: 4px;">
                            <i class="fas fa-check-circle" style="color: #28a745;"></i>
                            <strong style="color: #155724;">Consulta resuelta</strong>
                            <p style="margin: 0.5rem 0 0 0; color: #155724; font-size: 0.9rem;">
                                Tu consulta ha sido atendida por nuestro equipo.
                            </p>
                        </div>
                    ` : consulta.estado === 'en_proceso' ? `
                        <div style="margin-top: 1rem; padding: 0.75rem; background: #d1ecf1; border-left: 4px solid #17a2b8; border-radius: 4px;">
                            <i class="fas fa-hourglass-half" style="color: #17a2b8;"></i>
                            <strong style="color: #0c5460;">En proceso de revisión</strong>
                            <p style="margin: 0.5rem 0 0 0; color: #0c5460; font-size: 0.9rem;">
                                Nuestro equipo está trabajando en tu consulta.
                            </p>
                        </div>
                    ` : `
                        <div style="margin-top: 1rem; padding: 0.75rem; background: #fff3cd; border-left: 4px solid #ffc107; border-radius: 4px;">
                            ...
                            <p style="margin: 0.5rem 0 0 0; color: #856404; font-size: 0.9rem;">
                                Tu consulta será atendida a la brevedad.
                            </p>
                        </div>
                    `}
                    <div style="margin-top: 1rem; padding: 0.75rem; background: #e7f3ff; border-left: 4px solid #007cba; border-radius: 4px; text-align: center;">
                        ...
                        <strong style="color: #005a8b;">Clic para abrir el chat completo</strong>
                    </div>
                </div>
            `;
        }

        async abrirChatConsulta(consultaId, asunto) {
            this.currentConsultaId = consultaId;
            document.getElementById('chatConsultaTitulo').textContent = asunto;
            this.hideConsultasModal();

            const modal = document.getElementById('chatConsultaModal');
            if (modal) {
        ...
        async cargarMensajesChat() {
            try {
                console.log('Cargando mensajes para consulta ID:', this.currentConsultaId);
                ...
                if (!response || !response.data || response.data.length === 0) {
                    console.log('No hay mensajes o respuesta vacía');
                    container.innerHTML = `
                        <div style="text-align: center; padding: 2rem; color: #999;">
                            <i class="fas fa-comments" style="font-size: 3rem; margin-bottom: 1rem; opacity: 0.3;"></i>
                            <p>No hay respuestas aún. Espera a que el equipo responda.</p>
                        </div>
                    `;
                    return;
                }
        ...
        crearMensajeBurbuja(mensaje) {
            ...
            const alignment = esAdmin ? 'flex-start' : 'flex-end';
            ...
            return `
                <div style="display: flex; flex-direction: column; align-items: ${alignment};">
                    <div style="max-width: 70%; background: ${bgColor}; color: ${textColor}; padding: 0.75rem 1rem; border-radius: 12px; ${esAdmin ? 'border-bottom-left-radius: 4px;' : 'border-bottom-right-radius: 4px;'}">
                        <div style="font-size: 0.85rem; color: ${labelColor}; margin-bottom: 0.25rem; font-weight: 500;">
                            ${esAdmin ? '<i class="fas fa-user-shield"></i> Equipo de Soporte' : '<i class="fas fa-user"></i> Tú'}
                        </div>
